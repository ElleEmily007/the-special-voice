...
function OnboardingContent() {
  ...
  useEffect(() => {
    ...
    async function loadPlan() {
      ...
      try {
        ...
        if (!res.ok) {
          if (!cancelled) {
            setServerError(String(json.error ?? "Could not load your plan. Please contact support."));
          }
          return;
        }
        if (!cancelled) {
        ...
        if (!cancelled) setServerError("Network error loading your plan.");
      } finally {
  ...
  async function onSubmit(data: FormData) {
    ...
    try {
      ...
      setSavedTrialDays(json.trialDays ?? sessionPlan?.trialDays ?? null);
      setSubmitted(true);
    } catch {
  ...
  if (submitted) {
    const trialDays = savedTrialDays ?? sessionPlan?.trialDays;
    return (
        ...
        <div className="max-w-md w-full text-center">
          ...
          <div className="bg-[#0f2035]/5 rounded-xl px-5 py-4 text-left mb-6 space-y-2 text-sm">
            ...
            <p className="text-[#0f2035]/75">
              ✓ No charge until your{trialDays ? ` ${trialDays}-day` : ""} trial ends
            </p>
          </div>
          <a
            href="/"
    ...
    <div className="min-h-screen bg-[#fdf8ee] py-12 px-4">
      <div className="max-w-lg mx-auto">
        ...
        <div className="bg-white border border-[#0f2035]/10 rounded-2xl p-7 shadow-sm">
          {planLoading ? (
            <div className="flex items-center justify-center gap-2 py-8 text-[#0f2035]/50 text-sm">
            ...
            <form onSubmit={handleSubmit(onSubmit)} className="space-y-5">
              {sessionPlan && (
                <div className="bg-[#0f2035]/5 border border-[#0f2035]/10 rounded-xl px-4 py-3">
                  <p className="text-[#0f2035]/50 text-xs uppercase tracking-wide mb-1">
                    Your plan
                  </p>
                  <p className="text-[#0f2035] font-bold text-sm">
                    {sessionPlan.planName} — {sessionPlan.frequency}{" "}
                    {sessionPlan.frequency === 1 ? "story" : "stories"} per day
                  </p>
                  <p className="text-[#0f2035]/55 text-xs mt-1">
                    {sessionPlan.trialDays}-day free trial at your plan&apos;s frequency. Frequency
                    is set by the plan you purchased.
                  </p>
                </div>
              )}
              ...
              <div>
                ...
                <p className="text-[#0f2035]/40 text-xs mt-1.5">
                  Tap the play button to preview each voice. You selected:{" "}
                  {selectedVoice === "male" ? "Male (David)" : "Female (Sarah)"}.
                </p>
                {errors.voice && (
              ...
              <button
                type="submit"
                disabled={isSubmitting || !sessionPlan}
          ...
          {!planLoading && serverError && !sessionPlan && (
